...
variable "kubernetes_wk_subnet" {
  type = string

  validation {
    condition = can(cidrnetmask(var.kubernetes_wk_subnet))
    error_message = "Please provide valid CIDR for worker subnet."
  }
}
...
variable "workstation_subnet" {
  ...
  validation {
    condition = can(cidrnetmask(var.workstation_subnet))
